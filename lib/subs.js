...
export async function startSubBot(
  ...
  // --- INICIO DE CAMBIOS PARA PREMIUM (SIN ELIMINAR NADA) ---
...
const sock = makeWASocket({
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, isNewLogin, qr }) => {
    ...
    if (connection === 'open') {
      ...
      globalThis.db.data.settings[botDir].botprem = isPremium ? true : false // Cambio aquí
      globalThis.db.data.settings[botDir].type = isPremium ? 'Premium' : 'Sub' // Cambio aquí
...
delete commandFlags[senderId]
     ...
     // await joinChannels(sock)
      console.log(chalk.gray(`[ ✿  ]  ${isPremium ? 'PREMIUM-BOT' : 'SUB-BOT'} conectado: ${sock.userId}`))

      // --- LÓGICA DE TIEMPO (1 MES) ---
      if (isPremium) {
        if (!globalThis.db.data.settings[botDir].expiredAt) {
          globalThis.db.data.settings[botDir].expiredAt = Date.now() + (30 * 24 * 60 * 60 * 1000)
        }
        
        const timeLeft = globalThis.db.data.settings[botDir].expiredAt - Date.now()
        if (timeLeft <= 0) {
          console.log(chalk.red(`[ ! ] Sesión Premium expirada para ${sock.userId}`))
          await sock.logout()
    ...
    if (connection === 'close') {
      ...
      if ([401, 403].includes(reason)) {
        if (intentos < 5) {
          ...
          setTimeout(() => {
            startSubBot(m, client, caption, isCode, phone, chatId, commandFlags, isCommand) // Corregido para pasar flags
          }, 3000)
        } else {
          console.log(
